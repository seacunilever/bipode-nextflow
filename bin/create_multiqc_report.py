...
class BifrostData:
    ...
    def get_global_pod(self) -> Dict[str, Union[float, np.ndarray, int]]:
        ...
        quantiles[-1] -= 1e-6
        min_means, min_probes, min_cds = self.get_minimum_pod_means(quantiles)
        weights = self.get_confidence_threshold_probability_density(quantiles)
    ...
    def get_min_probe_weights(self, global_pod: Dict[str, Union[float, np.ndarray, int]]) -> Dict[str, np.ndarray]:
        ...
        weights = np.array(
            [
                np.sum(global_pod["weights"][global_pod["probes"] == i]) / np.sum(global_pod["weights"])
                for i in probes
            ]
        )
    ...
    def aggregate_probe_diagnostics(
        ...
        for probe in self.df["probes"]:
            probe_data = ProbeData(self.df, probe, conc_units)
            probe_diag_data = probe_data.get_diagnostic_data(cds_threshold, apply_cds_threshold)
...
class ProbeData:
    ...
    def create_probe_plot(self) -> str:
        ...
        if self._cache["cds"] > 0:
            ...
            if self.bifrost_data is not None:
                weights, bin_edges = self.bifrost_data.fit_pod_histogram(np.array(self.df[self.probe]["pod"]), self.df["n_samp"])
            else:
                # Fallback to direct calculation if no BifrostData instance
                pod_samples = np.array(self.df[self.probe]["pod"])
                n_samp = self.df["n_samp"]
                if np.isinf(pod_samples).any():
                    n_samp = len(pod_samples)
                pod_samples = pod_samples[~np.isinf(pod_samples)]
                n_pod_samples = len(pod_samples)
                if n_pod_samples == 0:
                    weights, bin_edges = None, None
                else:
                    prob_response = n_pod_samples / n_samp
                    counts, bin_edges = np.histogram(pod_samples, bins=int(np.sqrt(n_pod_samples)))
                    bin_size = bin_edges[1:] - bin_edges[:-1]
        ...
        fig.update_layout(
            xaxis=dict(
                title=dict(text=f"Concentration ({self.conc_units})", font=dict(size=12)),
                type="log",
                showgrid=True,
                gridcolor="#E5E5E5",
                gridwidth=1,
        ...
        logger.info(f"Completed concentration-response plot for {self.probe}")
        return plot_html
...
class BifrostMultiQCReport:
    def __init__(self, bifrost_data, **kwargs):
    ...
    def create_report(self):
        ...
        summary_data = {
            "Global PoD": {"Value": f"{self.data.get_global_pod()['global_pod']:.2g} {self.conc_units}"},
            "Maximum tested concentration": {
                "Value": f"{10**self.data.stats['max_conc']:.2g} {self.conc_units}"
            },
            "Number of probes analyzed": {"Value": str(len(self.data.stats["probe"]))},
            "Number of hits": {"Value": str(int(self.data.get_global_pod()["num_hits"]))},
            "Number of CDS>{self.cds_threshold} / CDS=1.0": {
                "Value": str(int(np.sum(self.data.stats["cds"] > self.cds_threshold)))
            },
            "Number of CDS=1.0": {"Value": str(int(np.sum(self.data.stats["cds"] == 1.0)))},
        }

        # Add minimum responding probe(s)
        weights = self.data.get_min_probe_weights(self.data.get_global_pod())
        ...
        if len(valid_probes) > 0:
            ...
            summary_data["Minimum responding probe"] = {
                "Value": f"{top_probe}, weight={top_weight:.2g}, CDS={top_cds:.2g}, Mean PoD={top_pod:.2g} {self.conc_units}"
            }
        ...
        if len(self.data.stats["l2fc"]) > 0:
            ...
            summary_data["Largest fold increase"] = {"Value": self.data.stats["probe"][max_fc_idx]}
            summary_data["Largest fold decrease"] = {"Value": self.data.stats["probe"][min_fc_idx]}
        ...
        pod_vs_fc_data = {
            "Probe": [
                ...
                for x, y, text in zip(
                    10 ** self.data.filter_summary_statistics(self.cds_threshold)["pod"],
                    self.data.filter_summary_statistics(self.cds_threshold)["l2fc"],
                    self.data.filter_summary_statistics(self.cds_threshold)["probe"],
                )
            ]
        }

        # Calculate y-axis limits
        if len(self.data.filter_summary_statistics(self.cds_threshold)["l2fc"]) > 0:
            ymin, ymax = (
                min(self.data.filter_summary_statistics(self.cds_threshold)["l2fc"].min(), -2) - 1,
                max(self.data.filter_summary_statistics(self.cds_threshold)["l2fc"].max(), 2) + 1,
            )
        else:
            ymin, ymax = -2, 2
        ...
        xmax = (
            10 ** np.array(self.data.filter_summary_statistics(self.cds_threshold)["conc"]).max() * 2
        ...
        pod_vs_fc_plot = scatter.plot(
            ...
            pconfig={
                ...
                "x_lines": [  # Add vertical lines using x_lines
                    {
                        "value": float(self.data.get_global_pod()["global_pod"]),
                        "color": "#FF0000",  # Red
                        "width": 1,
                        "dash": "solid",
                        "label": "Global PoD",
                ...
                + [
                    ...
                    for conc in 10 ** np.array(self.data.filter_summary_statistics(self.cds_threshold)["conc"])
                ],
        ...
        main_module.add_section(
            ...
            exposing {self.cell_type} cells for {self.timepoint} to {self.test_substance}. The BIFROST model
        ...
        main_module.add_section(
            ...
            <ul>
                ...
                <li><strong>Number of CDS>{self.cds_threshold} / CDS=1.0</strong>: Probes with strong concentration-dependent responses (CDS > {self.cds_threshold}) or maximal response (CDS = 1.0).</li>
        ...
        main_module.add_section(
            ...
            probe-level PoD (mean given response){" for probes with CDS > " + str(self.cds_threshold) if apply_cds_threshold else ""}. The red vertical line indicates the global PoD.
            ...
            <ul>
                ...
                {"<li>Only probes with CDS > " + str(self.cds_threshold) + " are shown, ensuring reliable concentration-dependent responses.</li>" if apply_cds_threshold else ""}
            </ul>
            """).strip(),
        )
        ...
        weighted_module = multiqc.BaseMultiqcModule(
            name=f'Probes with Non-zero Global PoD Weight{" (CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
            anchor="bifrost_weighted",
            info=f'Concentration-response plots for probes contributing to global PoD{" (filtered by CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
        )

        # Create module for fold change plots
        fc_module = multiqc.BaseMultiqcModule(
            name=f'Fold Change Plots{" (CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
            anchor="bifrost_fc_plots",
            info=f'PoD vs Fold Change plots{" for probes with CDS > " + str(self.cds_threshold) if apply_cds_threshold else ""}',
        )

        # Create module for lowest means plots
        lowest_means_module = multiqc.BaseMultiqcModule(
            name=f'Lowest Mean PoD Plots{" (CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
            anchor="bifrost_lowest_means_plots",
            info=f'Concentration-response plots for probes with lowest mean PoD{" (filtered by CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
        )
...
{self._get_plot_elements_description(self.cds_threshold, apply_cds_threshold)}
...
""").strip(),
        ...
        if len(probes_to_plot) > 0:
            ...
            weighted_table_data = self.data.create_summary_table_data(
                probes_to_plot, weights, self.conc_units, sort_by_abs_fc=True
            )
...
<p>The table shows key metrics for each probe, sorted by their weight in the global PoD calculation.</p>""".strip(),
            )

            for probe in probes_to_plot:
                probe_data = ProbeData(self.data.df, probe, self.conc_units, bifrost_data=self.data)
                conc_response_plot = probe_data.create_probe_plot()
                weighted_module.add_section(
                    name=probe,
                    anchor=f"bifrost_weighted_{probe}",
                    content=conc_response_plot,
                    description=f'CDS = {self.data.df[probe]["cds"]:.3f}, Mean PoD = {10**np.mean(self.data.df[probe]["pod"]):.2g} {self.conc_units}',
                )
...
<p><strong>Probe Selection:</strong></p>
<ul>
    <li>This section displays the {self.n_fold_change_probes} probes with the most extreme fold changes{" that meet two criteria:" if apply_cds_threshold else ":"}
        {"<ul>" if apply_cds_threshold else ""}
        {"<li>CDS > " + str(self.cds_threshold) + " (strong evidence for a concentration-dependent response)</li>" if apply_cds_threshold else ""}
    ...
    {"<li>Probes are first filtered to include only those with CDS > " + str(self.cds_threshold) + ", ensuring reliable concentration-dependent responses.</li>" if apply_cds_threshold else ""}
    <li>Among {"these filtered probes" if apply_cds_threshold else "all probes"}, the {self.n_fold_change_probes} with the most extreme fold changes are selected.</li>
    <li>If fewer than {self.n_fold_change_probes} probes meet these criteria, all qualifying probes are shown.</li>
</ul>
""").strip(),
        )
        ...
        fc_module.add_section(
            ...
            description=textwrap.dedent(f"""
            <p>Concentration-response plots for the {self.n_fold_change_probes} probes with the largest positive fold changes (increased expression).</p>
        ...
        if len(self.data.stats["l2fc"]) > 0:
            ...
            abs_fc = np.abs(self.data.stats["l2fc"])
            index = np.argsort(abs_fc)[::-1]  # Sort in descending order
            n_up = min(self.n_fold_change_probes, len(self.data.stats["l2fc"]))
            # Get probes with largest absolute fold changes that are positive
            up_probes = self.data.stats["probe"][index][self.data.stats["l2fc"][index] > 0][:n_up]
            ...
            up_table_data = self.data.create_summary_table_data(
                up_probes, weights, self.conc_units, sort_by_abs_fc=True
            )
...
""".strip(),
            ...
            for probe in up_probes:
                ...
                fc_module.add_section(
                    ...
                    content=conc_response_plot,
                    description=f'CDS = {self.data.df[probe]["cds"]:.3f}, Mean PoD = {10**np.mean(self.data.df[probe]["pod"]):.2g} {self.conc_units}, Log2 Fold Change = {self.data.stats["l2fc"][self.data.stats["probe"] == probe][0]:.2f}',
                )
        ...
        fc_module.add_section(
            ...
            description=textwrap.dedent(f"""
            <p>Concentration-response plots for the {self.n_fold_change_probes} probes with the largest negative fold changes (decreased expression).</p>
        ...
        if len(self.data.stats["l2fc"]) > 0:
            ...
            abs_fc = np.abs(self.data.stats["l2fc"])
            index = np.argsort(abs_fc)[::-1]  # Sort in descending order
            n_down = min(self.n_fold_change_probes, len(self.data.stats["l2fc"]))
            # Get probes with largest absolute fold changes that are negative
            down_probes = self.data.stats["probe"][index][self.data.stats["l2fc"][index] < 0][:n_down]
            ...
            down_table_data = self.data.create_summary_table_data(
                down_probes, weights, self.conc_units, sort_by_abs_fc=True
            )
...
""".strip(),
        ...
        if apply_cds_threshold:
            ...
            probes_to_plot = self.data.stats["probe"][mask][np.argsort(self.data.stats["pod"][mask])][
                : min(np.sum(mask), self.n_lowest_means)
            ]
        else:
            probes_to_plot = self.data.stats["probe"][np.argsort(self.data.stats["pod"])][
...
<p><strong>Probe Selection:</strong></p>
<ul>
    <li>This section displays the {self.n_lowest_means} probes with the lowest mean PoD values{" that meet two criteria:" if apply_cds_threshold else ":"}
        {"<ul>" if apply_cds_threshold else ""}
        {"<li>CDS > " + str(self.cds_threshold) + " (strong evidence for a concentration-dependent response)</li>" if apply_cds_threshold else ""}
        {"<li>Valid PoD estimate (mean PoD less than maximum tested concentration)</li>" if apply_cds_threshold else ""}
        {"</ul>" if apply_cds_threshold else ""}
    ...
    {"<li>Probes are first filtered to include only those with CDS > " + str(self.cds_threshold) + ", ensuring reliable concentration-dependent responses.</li>" if apply_cds_threshold else ""}
    <li>Among {"these filtered probes" if apply_cds_threshold else "all probes"}, the {self.n_lowest_means} with the lowest mean PoD values are selected.</li>
    <li>If fewer than {self.n_lowest_means} probes meet these criteria, all qualifying probes are shown.</li>
</ul>
""").strip(),
            )
...
""".strip(),
        ...
        stats_module = multiqc.BaseMultiqcModule(
            name=f'Probe-level PoD Statistics{" (CDS > " + str(self.cds_threshold) + ")" if apply_cds_threshold else ""}',
            anchor="bifrost_stats",
            info=f'Detailed statistics for probes{" with CDS > " + str(self.cds_threshold) if apply_cds_threshold else ""}',
        )
        ...
        if n_probe > 0:
            ...
            if apply_cds_threshold:
                ...
                probes = self.data.stats["probe"][cds_mask][np.argsort(self.data.stats["pod"][cds_mask])][
                    :self.n_pod_stats
                ]
                cds = self.data.stats["cds"][cds_mask][np.argsort(self.data.stats["pod"][cds_mask])][
                    :self.n_pod_stats
                ]
            else:
                probes = self.data.stats["probe"][np.argsort(self.data.stats["pod"])][:self.n_pod_stats]
                cds = self.data.stats["cds"][np.argsort(self.data.stats["pod"])][:self.n_pod_stats]
            logger.info(f"Found {len(probes)} probes to include in PoD statistics table")

            # Create table data
            table_data = {}
            for probe, cds_val in zip(probes, cds):
                n_pod_samples = len(self.data.df[probe]["pod"])
                extended_pod_samples = np.concatenate(
                    (
                        self.data.df[probe]["pod"],
                        [self.data.stats["max_conc"] for _ in range(self.data.stats["n_samp"] - n_pod_samples)],
                    )
                )
                ...
                for pod_val in pod_percentiles:
                    if pod_val < self.data.stats["max_conc"]:
            ...
            stats_module.add_section(
                ...
                description=f"""\
<p>Summary statistics for probes{" with CDS > " + str(self.cds_threshold) if apply_cds_threshold else ""}, showing the distribution of PoD (Point of Departure) values.</p>
<p>The table includes:</p>
<ul>
    <li><strong>CDS</strong>: Concentration-Dependency Score (probability of response below max concentration){" with threshold = " + str(self.cds_threshold) if apply_cds_threshold else ""}</li>
    <li><strong>PoD percentiles</strong>: Different quantiles of the PoD distribution</li>
    <li>Values are shown in {self.conc_units}</li>
    <li>Probes are sorted by median PoD (50th percentile)</li>
    <li>Only shows the top {self.n_pod_stats} probes{" with CDS > " + str(self.cds_threshold) if apply_cds_threshold else ""}</li>
</ul>
...
<ul>
    <li><strong>CDS > {self.cds_threshold}</strong>: Probes with strong concentration-dependent responses are prioritized</li>
    <li><strong>Lower Mean PoD</strong>: Among probes with CDS > {self.cds_threshold}, those with lower PoD (more sensitive) are shown first</li>
</ul>
...
""".strip(),
        ...
        try:
            ...
            multiqc.write_report(
                output_dir=os.path.dirname(self.output_name),
                filename=os.path.basename(self.output_name),
...
def main():
    args = parse_args()
    bifrost_data = BifrostData(args.summary_file)
    report = BifrostMultiQCReport(
        bifrost_data,
        test_substance=args.test_substance,
        cell_type=args.cell_type,
        timepoint=args.timepoint,
        conc_units=args.conc_units,
        output_name=args.output_name,
        interactive_plots=args.interactive_plots,
        n_fold_change_probes=args.n_fold_change_probes,
